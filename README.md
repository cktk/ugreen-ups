...
市电中断、UPS 仅靠电池供电时，可用 `-low` 与 `-action` 让电脑在电量耗尽前安全退场：

```bash
# 电量低于 20% 时自动关机（默认动作）
ups-monitor.exe -low 20

# 电量低于 15% 时睡眠（唤醒后可继续工作）
ups-monitor.exe -low 15 -action sleep

# 电量低于 10% 时休眠
ups-monitor.exe -web :8080 -low 10 -action hibernate
```
...
4. **动作可靠性**：`shutdown`(关机) 与 `hibernate`(休眠) 通过系统 `shutdown.exe` 执行，稳定可靠；`sleep`(睡眠) 调用 Windows `SetSuspendState` 电源 API，绝大多数环境可用，个别环境失败时会回退到 `rundll32` 并写入事件日志。

> 建议在断电演练中验证：临时把 `-low` 设到略高于当前电量（例如当前 100% 时设 `-low 99`），观察是否按预期关机/睡眠，确认无误后再设为真正想要的保护阈值。
